"""Mark explicit owner tasks due and send one allowlisted Telegram reminder."""
...
def maybe_notify_due_owner_tasks(
    ...
    *,
    ...
    if (
        ...
        and not store.has_owner_notification_delivery_claim(
    ...
    if not claimed_recipients:
        return 0
    # The transport can acknowledge a request before the caller's transaction is
    # committed (and may be followed by a rollback). Commit the recipient claim as
    # its own durable receipt before any network call so accepted/ambiguous sends
    # cannot be repeated after a crash or outer rollback.
    ...
    store.record_owner_notification_recipient_delivery_outcomes_durably(
        ...
        lead_id=_DUE_REMINDER_LEAD,
        notification_key=day,
        delivered_recipient_ids=tuple(delivery.delivered),
        rejected_recipient_ids=tuple(delivery.rejected),
    )
...
def main() -> None:
    ...
    logger.info(
        "due_scan complete owner_tasks_scanned=%s owner_tasks_due_ready=%s "
        "owner_reminders_sent=%s",
