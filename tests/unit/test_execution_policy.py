...
def test_owner_agent_is_ai_automatic_fail_closed() -> None:
    policy = policy_for(CapabilityId.OWNER_AGENT)
    assert policy.execution_mode == ExecutionMode.AI_AUTOMATIC
    assert policy.fail_closed is True
...
def test_registry_covers_all_capability_ids() -> None:
    registry_keys = {policy_for(cap).capability for cap in CapabilityId}
