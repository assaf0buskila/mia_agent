...
def test_fallback_reason_is_recorded_so_the_failure_is_visible() -> None:
    ...
    # failure line with a failure *class*, not the caller's canned "CANNED" fallback.
...
def test_owner_agent_ready_when_only_the_sales_model_is_configured() -> None:
    ...
    settings = Settings(
        ...
        openai_api_key="k",
        owner_agent_model="",
        owner_agent_fallback_model="",
        sales_model="sales-live",
    ...
    assert blank.owner_agent_ready() is False


def test_empty_reply_advances_to_the_next_model() -> None:
    """A 200 with no content is how some model ids fail live. Try the fallback."""
    chain = LlmModelChain([_client("empty", _ok("")), _client("good", _ok("hi"))])
    assert chain.complete(messages=_messages()).text == "hi"
    assert chain.last_model == "good"
...
def test_content_parts_are_read_as_text() -> None:
    ...
    chain = LlmModelChain([_client("parts", handler)])
    assert chain.complete(messages=_messages()).text == "שלום"


def test_a_tool_payload_400_advances_instead_of_killing_the_console() -> None:
    """400 without tools is a payload bug. 400 *with* tools is often this model rejecting
