...
def test_env_example_documents_settings_and_adapter_map() -> None:
    ...
    missing = [
        f"MIA_{name.upper()}" for name in Settings.model_fields if f"MIA_{name.upper()}" not in text
    ]
    assert missing == []
    assert "MIA_INSTAGRAM_SENDER=direct" in text
    assert "MIA_INSTAGRAM_SENDER=composio" not in text
    assert "MIA_WHATSAPP_CLICK_TO_CHAT=" in text
...
def test_ecs_service_example_is_private_fargate() -> None:
    ...
    assert not (ROOT / "deploy/ecs-reconcile-overrides.example.json").exists()
    task_path = ROOT / "deploy/ecs-task-definition.example.json"
    task = json.loads(task_path.read_text(encoding="utf-8"))
...
def test_ci_builds_production_image() -> None:
    ...
    assert (
        'docker build -f deploy/Dockerfile --build-arg MIA_BUILD_SHA="${{ github.sha }}" '
        "-t mia:ci ."
    ) in workflow
    dockerfile = (ROOT / "deploy/Dockerfile").read_text(encoding="utf-8")
    assert 'ARG MIA_BUILD_SHA=""' in dockerfile
    assert "org.opencontainers.image.revision=${MIA_BUILD_SHA}" in dockerfile
    assert "ENV MIA_BUILD_SHA=${MIA_BUILD_SHA}" in dockerfile
...
def test_production_templates_preserve_network_database_and_alarm_boundaries() -> None:
    alb_sg = json.loads((ROOT / "deploy/sg-alb-ingress.example.json").read_text(encoding="utf-8"))
    task_sg = json.loads(
        (ROOT / "deploy/sg-tasks-ingress.example.json").read_text(encoding="utf-8")
    )
    rds_sg = json.loads((ROOT / "deploy/sg-rds-ingress.example.json").read_text(encoding="utf-8"))
    ...
    acm = json.loads((ROOT / "deploy/acm-certificate.example.json").read_text(encoding="utf-8"))
    ...
    five = json.loads((ROOT / "deploy/cloudwatch-alb-5xx.example.json").read_text(encoding="utf-8"))
    ...
    assert five["AlarmActions"] == ["arn:aws:sns:REGION:ACCOUNT_ID:mia-alb-5xx"]
...
def test_fill_placeholders_replaces_tokens_on_windows() -> None:
    ...
    task = json.loads((ROOT / "deploy/local/ecs-task-definition.json").read_text(encoding="utf-8"))
    assert "111122223333" in task["executionRoleArn"]
    secrets = {
        item["name"]: item["valueFrom"] for item in task["containerDefinitions"][0]["secrets"]
    }
    assert secrets["MIA_INSTAGRAM_ACCOUNT_ID"].endswith("mia/prod:MIA_INSTAGRAM_ACCOUNT_ID::")
    assert (
        task["containerDefinitions"][0]["logConfiguration"]["options"]["awslogs-region"]
        == "il-central-1"
    )
    tg = json.loads((ROOT / "deploy/local/alb-target-group.json").read_text(encoding="utf-8"))
    assert tg["VpcId"] == "vpc-aabbccdd"
    unhealthy = json.loads(
        (ROOT / "deploy/local/cloudwatch-alb-unhealthy.json").read_text(encoding="utf-8")
    )
    five = json.loads((ROOT / "deploy/local/cloudwatch-alb-5xx.json").read_text(encoding="utf-8"))
