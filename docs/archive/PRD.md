...
| Surface | Role |
| --- | --- |
    | Website | Primary customer sales. One bottom control: visible Ask Mia pill (opens on click). Mic lives in the open composer (voice in, STT, same sales graph). No TTS. No competing WhatsApp FAB. Chat uses sent/received bubbles with avatars. Customer Hebrew is 2nd-person plural / impersonal (men and women), native, and uses no hyphen/dash. Mia answers only published shop facts, one question at a time, hands off on money/promise/complaint/human request, and does not chase after a day of silence unless the opener is approved. She now **answers before she asks** (`sales_reply_v9`, ADR-028 + ADR-040): published facts reach the customer path through knowledge-only retrieval that can never read owner memory. The **booked meeting is the default exit** (`MIA_WEBSITE_MEETING_FIRST`, default true); WhatsApp is the fallback once the meeting was offered and not taken, or on explicit request, after real context (ADR-018), typically 3–6 meaningful exchanges. Public chat POSTs (session, message, voice, handoff, end) are origin-bound to `MIA_CORS_ORIGINS` plus the public host and rate-limited; missing or unknown Origin is rejected. CORS in `app/main.py` remains the browser gate; the widget still posts with `credentials: 'omit'`. |
...
| LinkedIn | Profile via Composio (`LINKEDIN_GET_MY_INFO`). Personal post analytics is optional Direct REST + `MIA_LINKEDIN_ACCESS_TOKEN` (ADR-009/034 — optional, not a go-live requirement). Composio has org share-stats only — do not fake member analytics. No posts/DMs. |
...
- Region `eu-north-1`. Host `https://mia.assafweb.com`. Live image `mia:20` (task `mia:22`). `/health` includes `brain` + `owner_integrations` + `gmail_inbox`. Brain env: `MIA_OWNER_AGENT_MODEL=gpt-5.6-luna`, `MIA_OWNER_AGENT_FALLBACK_MODEL=gpt-5.6-terra`, `MIA_EXTRACTION_MODEL=gpt-5.6-luna`, `MIA_EMBEDDING_MODEL=text-embedding-3-small`. `MIA_GMAIL_SEND=false`. `MIA_COMPOSIO_DISCOVERY` defaults false. GSC / GA4 / Meta ads ids are leftover env, optional only when discovery is on. `MIA_SHEETS_SPREADSHEET_ID` stays required; `MIA_LINKEDIN_ACCESS_TOKEN` is optional leftover (ADR-034). Firecrawl is not Composio.
- Research: Firecrawl primary; Apify `google-search-scraper` fallback via `MIA_APIFY_TOKEN` when Firecrawl is empty (ADR-035).
