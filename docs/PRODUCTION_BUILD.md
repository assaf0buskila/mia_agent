...
| Calendar writes | `MIA_CALENDAR_WRITE=true` (Assaf ADOPT 2026-08-22). Gmail send after Approve (ADR-033). Meta writes stay off |
| Gmail / Meta send | Gmail send **on after Approve** (`MIA_GMAIL_SEND=true`). Meta writes stay **off**. R4/R5 are not env knobs. Follow-up persist/due-scan is alive; no auto-send flag. |
