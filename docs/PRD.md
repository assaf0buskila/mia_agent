...
| LinkedIn | Profile via Composio (`LINKEDIN_GET_MY_INFO`). Personal post analytics has no Composio member tool; leftover Direct REST token is unused. No posts/DMs. |
...
- Region `eu-north-1`. Host `https://mia.assafweb.com`. Live image `mia:15`. Working tree `/health` adds `brain` + `owner_integrations`. Those keys are not on mia:15 yet. `owner_integrations.missing` does not ask for GSC/GA4/Sheets/LinkedIn token — Composio lists those. Firecrawl is the extra env.
